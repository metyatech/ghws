...
if [[ -z "\${GHWS_MOBILE_E2E_MENU_ACTIVE:-}" ]]; then
  ...
  if [[ -n '${AUTO_RESUME_SESSION}' ]]; then
    ...
    exec /mnt/d/ghws/scripts/wsl-agent-mobile-menu.sh resume '${AUTO_RESUME_SESSION}' 2>>'$LOGIN_HOOK_LOG_PATH'
  fi
  exec /mnt/d/ghws/scripts/wsl-agent-mobile-menu.sh 2>>'$LOGIN_HOOK_LOG_PATH'
fi
EOF
