...
TMUX_ATTACHED_FLAG_PATH = "/tmp/ghws-mobile-e2e/tmux-attached.flag"
...
def prepare_mobile_test_session(catalog_path: str) -> str:
    ...
    result = run_command(
        [
            ...
            "/mnt/d/ghws/scripts/wsl-agent-mobile-menu.sh",
            "start",
            "shell",
            title,
        ],
...
def start_temp_sshd(catalog_path: str, session_name: str) -> None:
    run_command(
        [
            ...
            "root",
            "--",
            "/mnt/d/ghws/scripts/wsl-mobile-e2e-sshd.sh",
            "start",
            "--port",
            str(TEMP_SSH_PORT),
            "--user",
...
def get_sshd_log() -> str:
    result = wsl_root_bash("cat /tmp/ghws-mobile-e2e/sshd.log 2>/dev/null || true", check=True, timeout=30)
    return result.stdout


def get_login_hook_log() -> str:
    result = wsl_root_bash("cat /tmp/ghws-mobile-e2e/login-hook.log 2>/dev/null || true", check=True, timeout=30)
    return result.stdout
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="ghws-android-mobile-e2e-"))
