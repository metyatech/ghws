...
try {
    & $launcherScript -Mode new -Type shell -Name $primaryLabel -Title $primaryTitle -WorkingDirectory 'D:\ghws' -Detach | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw 'Failed to create the primary PC path-matrix session.'
    }

    Assert-SessionVisible -TargetSessionName $primarySessionName -ExpectedTitle $primaryTitle -ExpectedFolder 'D:\ghws'
    Assert-ResumeAvailable -TargetSessionName $primarySessionName

    & $launcherScript -Mode new -Type shell -Name $secondaryLabel -Title $secondaryTitle -WorkingDirectory 'D:\ghws\scripts' -Detach | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw 'Failed to create the secondary PC path-matrix session.'
    }

    Assert-SessionVisible -TargetSessionName $secondarySessionName -ExpectedTitle $secondaryTitle -ExpectedFolder 'D:\ghws\scripts'
