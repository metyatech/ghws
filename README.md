...
Each repository is identified by its workspace-relative path and full absolute path in both the per-repo progress output and the final summary, so that repositories with the same leaf name are unambiguous (e.g. `parent-a\repo  OK  [C:\ghws\parent-a\repo]`).
Submodules (`.git` is a file) are detected and skipped automatically; the skip and summary lines include the relative path so the parent directory context is visible. Repositories whose current branch has no upstream, or whose local changes block a pull, are also reported as `SKIP (...)` rather than hard failures. Internal `.git` storage paths (e.g. `.git/modules/...`) are excluded so they are never mistaken for repositories, and transient/inaccessible directories found during recursive discovery are suppressed from the live output and summarized once instead of spamming errors. Non-git directories are silently ignored. If any repository still fails to pull after those skip classifications, it is reported and the script exits with code 1 after finishing all others.
