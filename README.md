...
Submodules (`.git` is a file) are detected and skipped automatically; the skip and summary lines include the relative path so the parent directory context is visible. Repositories whose current branch has no upstream are reported as `NOTE (no upstream)` and do not fail the overall run. Repositories whose local changes block a pull are still treated as failures. Internal `.git` storage paths (e.g. `.git/modules/...`) are excluded so they are never mistaken for repositories, and transient/inaccessible directories found during recursive discovery are suppressed from the live output and summarized once instead of spamming errors. Non-git directories are silently ignored. If any repository fails to pull, it is reported and the script exits with code 1 after finishing all others.
