...
Each repository is identified by its workspace-relative path and full absolute path so that repositories with the same leaf name are unambiguous (e.g. `parent-a\repo [C:\ghws\parent-a\repo]`).
