...
Submodules (`.git` is a file) are detected and skipped automatically; the skip and summary lines include the relative path so the parent directory context is visible. Internal `.git` storage paths (e.g. `.git/modules/...`) are excluded so they are never mistaken for repositories. Non-git directories are silently ignored. If any repository fails to pull, it is reported and the script exits with code 1 after finishing all others.
